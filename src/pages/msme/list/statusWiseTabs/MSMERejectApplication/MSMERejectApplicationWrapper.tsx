...
import MSMERejectApplication from "./MSMERejectApplication";
...
import {
  ...
  setItems,
  setTotalItems,
} from "src/redux/slices/MSMEApplication/MSMERejectedApplicationSlice";
...
const MSMERejectApplicationWrapper = () => {
  const navigate = useNavigate();
  const dispatch = useDispatch<AppDispatch>();
  // Slice
  const MSMERejectApplicationState: any = useSelector(
    (state: RootState) => state.MSMERejectedApplication
  );
  const { page, rowsPerPage, items, searchValue, filterBy } =
    MSMERejectApplicationState;
  const [changeStatus] = useChangeMSMEStatusMutation();
  // Get Data Query
  const { data, isFetching, isLoading } = useGetMSMERejectApplicationsQuery({
    limit: rowsPerPage,
...
export default MSMERejectApplicationWrapper;
