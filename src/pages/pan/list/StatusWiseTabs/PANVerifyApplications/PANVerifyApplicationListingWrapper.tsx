...
import {
  ...
  setItems,
  setTotalItems,
} from "src/redux/slices/PANApplication/PANVerifiedApplicationSlice";
...
import PANVerifyApplicationListing from "./PANVerifyApplicationListing";
...
const PANVerifyApplicationListingWrapper = () => {
  // Dispatch Hook
  const dispatch = useDispatch<AppDispatch>();
  const navigate = useNavigate();
  // Slice
  const PANVerifiedApplicationState: any = useSelector(
    (state: RootState) => state.PANVerifiedApplication
  );
  const { items, searchValue, rowsPerPage, page, filterBy, dateFilter , orderByValue } =
    PANVerifiedApplicationState;
  ...
  // Get Data Query
  const { data, isFetching, isLoading } = useGetPanVerifiedApplicationsQuery({
    limit: rowsPerPage,
          ...
          <button
            ...
            onClick={(e) => {
              ...
              showConfirmationDialog({
                title: "Heads Up",
                text: "Are you sure want to Generate ?",
                icon: "question",
                showCancelButton: true,
                next: (result: any) => {
                  if (result.isConfirmed) {
                    const formData = new FormData();
                    formData.append("requestedStatus", "GENERATE");
            ...
            className="bg-status-generate text-white rounded px-2 py-1 font-medium"
          >
            {" "}
            Generate
          </button>
...
export default PANVerifyApplicationListingWrapper;
